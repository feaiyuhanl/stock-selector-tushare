...
- **TOP结果输出**：自动筛选并输出评分最高的股票，包含板块和概念信息，同时输出板块趋势涨幅TOP 3和热点概念TOP 10
- **本地缓存机制**：所有数据（K线、基本面、财务、板块概念）自动缓存到本地Excel，大幅提升运行速度；K线数据支持增量更新和自动清理
...
============================================================
     code    name  total_score  fundamental_score  volume_score  price_score  sector_score  concept_score  sectors  concepts
1  000001  平安银行     85.32             78.50         92.30         88.20          82.10           79.50     银行    5G,人工智能,新能源
2  000002  万科A       84.15             75.20         90.10         85.30          80.50           78.20     房地产  房地产,物业管理
...

============================================================
【板块趋势涨幅TOP 3】
------------------------------------------------------------
1. 银行: 涨幅 5.23% (涉及股票: 15只, 平均评分: 82.50)
2. 房地产: 涨幅 4.56% (涉及股票: 12只, 平均评分: 80.30)
3. 计算机: 涨幅 3.89% (涉及股票: 20只, 平均评分: 78.90)
============================================================

【热点概念TOP 10】
------------------------------------------------------------
1. 5G: 平均评分 85.20 (涉及股票: 35只, 热度: 0.852)
2. 人工智能: 平均评分 82.50 (涉及股票: 28只, 热度: 0.825)
3. 新能源: 平均评分 80.30 (涉及股票: 45只, 热度: 0.803)
...
============================================================
【评分说明】
------------------------------------------------------------
...
- **板块趋势**（权重50%）：板块整体走势（基于申万行业指数）
- **相对强度**（权重50%）：股票相对板块的表现

**板块说明**：
- 板块指股票的行业分类（如"银行"、"房地产"、"计算机"等）
- 一只股票通常只有一个主要板块
- 板块K线数据通过匹配申万行业指数（SW指数）获取
...
**概念说明**：
- 一只股票可能涉及多个概念（如"5G"、"人工智能"、"新能源"等）
- 概念K线数据通过概念成分股计算：取概念市值TOP20股票（不足20只取全部），使用等权重平均计算概念指数
- 如果股票有多个概念，程序会计算每个概念的得分，然后取所有概念得分的平均值作为最终概念评分
...
| 数据类型 | 缓存有效期 | 说明 |
|---------|-----------|------|
| **K线数据** | 1天（当日缓存） | 每日更新，检查最新交易日数据；自动保留最近250个交易日（约1年） |
| **股票列表** | 1天 | 每日更新 |
| **基本面数据** | 7天 | 相对稳定，每周更新 |
...
**注意**：K线数据会自动清理，只保留最近250个交易日（约1年）的数据，可通过 `config.py` 中的 `KLINE_CACHE_RETENTION_DAYS` 配置项调整保留天数。
...
1. **独立文件存储**（优先）：
   - 每个股票的数据存储在独立文件中
   - K线数据路径：`cache/data/kline/stock_{stock_code}_{period}.xlsx`（如：`stock_000002_daily.xlsx`）
   - 其他数据路径：`cache/data/{type}/{stock_code}.xlsx`
   - 优点：支持断点续传，便于增量更新
...
| **日常运行** | 根据有效期自动刷新过期数据；K线数据增量更新（只获取新数据） |
...
- **自动清理**：保存时自动清理超过保留天数的旧数据，默认保留最近250个交易日（约1年）
- **日期格式**：Excel中保存的日期只包含日期部分，不包含时间（00:00:00）
...
| **ConceptScorer** | ❌ 不可用（需2000积分） | ✅ 完全可用 | 通过概念成分股计算（市值TOP20等权重平均） |
...
5. **板块/概念数据**：
   - **板块数据**：通过申万行业指数获取，一只股票通常只有一个板块
   - **概念数据**：通过概念成分股计算，取市值TOP20股票（不足20只取全部）等权重平均计算概念指数；一只股票可能有多个概念，程序会计算所有概念的平均得分
   - 如果板块/概念K线数据获取失败，使用默认分50分参与计算
