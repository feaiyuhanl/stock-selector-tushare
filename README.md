...
- **多维度评分**：综合考虑基本面、成交量、成交价格、关联板块走势等维度
- **子维度细化**：每个大维度下包含多个子维度，评分更加精准
- **权重可配置**：支持自定义各维度权重，灵活调整选股策略
- **TOP结果输出**：自动筛选并输出评分最高的股票，包含板块信息，同时输出板块趋势涨幅TOP 3
- **本地缓存机制**：所有数据（K线、基本面、财务、板块）自动缓存到本地Excel，大幅提升运行速度；K线数据支持增量更新和自动清理
...
│
└── cache/                    # 缓存目录（自动创建）
    ├── kline/                # K线数据目录（所有K线数据统一放在这里）
    │   ├── stock_{code}_{period}.xlsx  # 股票K线数据
    │   └── sector_{name}_{period}.xlsx  # 板块K线数据
    └── meta/                  # 元数据目录（集中存储文件）
...
============================================================
     code    name  total_score  fundamental_score  volume_score  price_score  sector_score  sectors
1  000001  平安银行     85.32             78.50         92.30         88.20          82.10     银行
2  000002  万科A       84.15             75.20         90.10         85.30          80.50     房地产
...
...
【评分说明】
------------------------------------------------------------
总评分 = 基本面评分 × 35.0% + 成交量评分 × 25.0% + 价格评分 × 25.0% + 板块走势评分 × 15.0%
============================================================
```
...
- 通过 `index_classify` + `index_member` 建立股票到行业的映射表，确保数据准确
- 板块K线数据直接使用行业指数代码获取，避免名称匹配失败
- 一只股票可能属于多个行业（如果该股票在多个行业指数的成分股中）
...
- **板块数据缺失**：板块K线缺失时使用默认分50分；如果所有股票都缺失板块数据，板块权重设为0
...
| **板块信息** | 7天 | 超过7天自动失效，重新获取（行业分类变化频率低） |
| **行业映射表** | 首次运行时加载 | 使用 `index_classify` + `index_member` 建立股票到行业的映射表 |

**注意**：
- 缓存有效期是指缓存文件的有效期，超过有效期后会自动失效并重新获取数据
...
- **板块信息**：缓存有效期7天，超过7天自动失效并重新获取（行业分类变化频率很低）
- **行业映射表**：首次运行时自动加载，使用 `index_classify` + `index_member` 建立股票到行业的映射表，后续直接查表，查询速度快
...
1. **K线数据存储**（独立文件）：
   - 每个股票/板块的K线数据存储在独立文件中
   - K线数据路径：`cache/kline/{type}_{symbol}_{period}.xlsx`
     - 股票：`cache/kline/stock_{stock_code}_{period}.xlsx`（如：`stock_000002_daily.xlsx`）
     - 板块：`cache/kline/sector_{sector_name}_{period}.xlsx`
   - 优点：支持断点续传，便于增量更新

2. **元数据存储**（集中文件）：
   - 基本面、财务、板块等元数据汇总在meta目录下的集中文件中
...
3. **数据更新**：财务数据、板块数据变化频率低，缓存有效期7天；如需立即获取最新数据，使用 `--refresh` 参数强制刷新
4. **缓存清理**：
...
| `index_classify` | 获取行业分类 | 2000 | 用于建立股票到行业的映射表 |
| `index_member` | 获取指数成分股 | 2000 | 用于建立股票到行业的映射表 |
| `index_basic` | 获取指数基本信息 | 2000 | 用于板块K线（兼容旧代码） |
...
- 更新文档，移除概念评分相关描述（注：代码中仍保留概念数据获取功能，但评分流程中未使用）
